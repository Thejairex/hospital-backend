...
traffic = json.load(open('sql/Importaciones/dataEnfermero.json'))
columns = ['dni_enfermero', 'nombre', 'apellido', 'sexo', 'telefono','fecha_nac','estado']

for row in traffic:
    keys= tuple(row[c] for c in columns)
    cursor.execute("INSERT INTO enfermero VALUES(%s,%s,%s,%s,%s,%s,%s)",keys)
